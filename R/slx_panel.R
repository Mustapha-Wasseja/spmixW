...
#' wrapper around \code{\link{ols_panel}} that augments X with WX.
#'
#' \deqn{y = X \beta + W X \theta + \text{FE} + \varepsilon}
#'
#' No spatial autoregressive parameter is estimated. Effects decomposition:
#' direct = \eqn{\beta}, indirect = \eqn{\theta}, total = \eqn{\beta + \theta}.
#'
#' @inheritParams sar_panel
#'
#' @return An S3 object of class \code{"spmixW"} with OLS fields plus
...
#'
#' @export
slx_panel <- function(y, X, W, N, Time, ndraw = 5500L, nomit = 1500L,
                      prior = list()) {
  ...
  # Call ols_panel with augmented X
  result <- ols_panel(y, xmat, N, Time, ndraw, nomit, prior)

  # Update method label
  result$meth <- switch(as.character(prior$model %||% 0L),
    "0" = "pslx_g", "1" = "slxsfe_g", "2" = "slxtfe_g", "3" = "slxstfe_g"
  )

  # SLX effects: direct = beta, indirect = theta, total = beta + theta
